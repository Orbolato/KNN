...
inputs[[2]]=c("CG","Qtn","Fr_p","U2")
inputs[[3]]=c("Depth_m","qt_MPa","fs_kPa","u_kPa")
inputs[[4]]=c("CG","Depth_m","qt_MPa","fs_kPa","u_kPa")
  ...
  for(out in c("MSBTn","EST")){
    ...
    for(inpCase in 1:length(inputs)){
      ...
      for(i in 1:10){
        ...
        if(out=="EST"){
          mtest=mtest[mtest[,out]!=-1,]
          mvalid=mvalid[mvalid[,out]!=-1,]
          mtrain=mtrain[mtrain[,out]!=-1,]
        }
      ...
      save.image("C:/Users/aluno/Desktop/DADOS/saved.RData")
    }
  }
}
