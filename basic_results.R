...
path1 = file.path("C:","Users","ORBOLATO","Desktop","Mestrado","Dados","Base integrada", "geology.csv")
dataGeo = read.table(path1, header = TRUE, sep = ",", stringsAsFactor = FALSE)

path2 = file.path("C:","Users","ORBOLATO","Desktop","Mestrado","Dados","Base integrada", "complete.csv")
...
##############################################################################
# Defining validation sets for each test stage without reposition
...
finputs=c("Depth_m","qt_MPa","fs_kPa","u_kPa")
finputs=cbind(finputs,c("Depth_m","Qt1","Fr_p","Bq"))
finputs=cbind(finputs,c("Depth_m","Qtn","Fr_p","U2"))
...
for(mode in c("rectangular","gaussian")){
  ...
  for(out in c("SBTn","MSBTn")){
    ...
    for(inpcol in 1:ncol(finputs)){
      
      # For each number of input attributes
      
      for(nrows in 4:nrow(finputs)){
        
        # Generating combinations of input attributes
        
        inputs=combn(finputs[,inpcol],nrows)
        ...
        for(inpCase in 1:ncol(inputs)){
          ...
          for(i in 1:10){
            ...
            cat("Unbalanced training set pre-test accuracy:",mp,"\n\n")
